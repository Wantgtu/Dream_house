...
export default class LobbyC extends BaseController {
    ...
    intoLayer() {
        ...
        if (flag) {
            ...
            LoadingC.instance().addItem('data/game_info', ResType.Json, ModuleID.RES, '加载游戏配置信息')
            LoadingC.instance().addItem('data/activity_info', ResType.Json, ModuleID.RES, '加载活动配置信息')
            LoadingC.instance().addItem('data/game_lang_zh', ResType.Json, ModuleID.RES, '加载文本配置信息')

            LoadingC.instance().addItem(ModuleID.PUBLIC, ResType.AssetBundle, ModuleID.RES, '加载公共分包')
            LoadingC.instance().addItem(ModuleID.CRAZY_CLICK, ResType.AssetBundle, ModuleID.RES, '加载砸蛋分包')
            LoadingC.instance().addItem(ModuleID.GAME, ResType.AssetBundle, ModuleID.RES, '加载游戏分包')
            LoadingC.instance().addItem(ModuleID.AUDIO, ResType.AssetBundle, ModuleID.RES, '加载音频分包')
            if (StartC.instance().isState(ItemState.NOT_GET)) {
                LoadingC.instance().addItem(ModuleID.START, ResType.AssetBundle, ModuleID.RES, '加载初始分包')
            }
            console.log('SDKManager.getChannel().hasNativeAd() ', SDKManager.getChannel().hasNativeAd())
            if (SDKManager.getChannel().hasNativeAd()) {
                LoadingC.instance().addItem(ModuleID.nativeAd, ResType.AssetBundle, ModuleID.RES, '加载nativeAd分包')
            }
            if (CMgr.helper.hasInstallApp()) {
                LoadingC.instance().addItem(ModuleID.install, ResType.AssetBundle, ModuleID.RES, '加载install分包')
            }
            LoadingC.instance().addItem('public/public', ResType.SpriteAtlas, ModuleID.PUBLIC, '加载共用大图')
            LoadingC.instance().addItem('texture/props', ResType.SpriteAtlas, ModuleID.GAME, '加载游戏大图')
            LoadingC.instance().addItem('bg/person', ResType.SpriteAtlas, ModuleID.GAME, '加载角色头像大图')
