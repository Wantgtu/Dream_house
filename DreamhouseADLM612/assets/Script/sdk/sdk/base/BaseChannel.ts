import { ADDir, ChannelID, DataCallback, getChannel, NativeAdCallback, ResultCallback, ResultState, SDKDir, SDKState } from "../SDKConfig";
...
export default class BaseChannel extends BaseSDK {
    ...
    showRewardAd(site: number, callback: ResultCallback) {
        ...
        //统一特殊处理 穿山甲采用随机方式传入广告id
        if (SDKManager.getChannel().channelID == ChannelID.CSJ) { 
            site = this.getRandomInt(0, this.rewardAd.length);
        }
